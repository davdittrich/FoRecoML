...
# Use rml_g() directly to obtain an rml_g_fit; csrml_g() returns the
# reconciled matrix, not the fit object.
...
make_s3_fixture <- function(p = 4L, T_obs = 20L, ncol_hat = 5L, seed = 77L) {
  ...
  list(obs = obs, hat = hat)
}

test_that("(a) predict.rml_g_fit returns numeric vector", {
  skip_if_not_installed("ranger")
  fx  <- make_s3_fixture()
  fit <- rml_g(approach = "ranger", hat = fx$hat, obs = fx$obs, seed = 1L)
