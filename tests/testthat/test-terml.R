...
if(require(testthat)){
  ...
  test_that("Approach and features", {
    for(i in c("xgboost", "mlr3", "lightgbm", "randomForest")){
      ...
      expect_no_error(terml(hat = hat, obs = obs, base = base,
                            agg_order = m, approach = i, seed = 123,
                            features = j))
    }
  })

  test_that("Two step", {
    mdl <- terml(hat = hat, obs = obs,
                 agg_order = m, approach = "lightgbm",
                 seed = 123, features = "all")
    r1 <- terml(hat = hat, obs = obs, base = base,
                agg_order = m, approach = "lightgbm",
                seed = 123, features = "all")
